...
class ChaChing(object):
    ...
    def my_orders(self, market):
        response = self.exchange.myOrders(MARKET_IDS[market])
        if response['success'] == '1':
            ...
            raise Exception("Failed retrieving orders!")
        return response

    def fetch_all_markets(self):
        try:
            market_data = self.exchange.getMarketDataV2()
    ...
    def fetch_market(self, market):
        try:
            market_data = self.exchange.getSingleMarketData(MARKET_IDS[market])
        except urllib2.HTTPError:
            raise
        else:
...
class TradeBot2000(ChaChing):
    ...
    """
    def trade(self):
        print self.get_market('DOGE/LTC')
